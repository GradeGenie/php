    ...
    $data = [
        ...
        'messages' => [
            [
                'role' => 'system',
                'content' => '
                You are a grading assistant for a teacher, providing accurate grading comments, grades and scores for provided assignments.',
            ],
            [
                'role' => 'user',
                'content' => "Take the extracted contents of this PDF/DOCX student assignment and return grades. \n\n$content",
            ],
        ],
        'max_tokens' => 2500,
    ];
...
try {
    ...
    while ($job = $pheanstalk->reserveWithTimeout(0)) { // Reserve with timeout to exit if no jobs are available
        ...
        $prompt .= json_encode([
            "grade" => "A",
            "score" => "50",
            "comments" => "longform comments here, use HTML for formatting if needed"
        ], JSON_PRETTY_PRINT);
