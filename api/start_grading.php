...
try {
    ...
    if (isset($_FILES['files'])) {
        ...
        for ($i = 0; $i < $totalFiles; $i++) {
            ...
            $response['debug'][] = "Temporary file path: " . $_FILES['files']['tmp_name'][$i];
            $response['debug'][] = "Upload error code: " . $_FILES['files']['error'][$i];
            ...
            if ($_FILES['files']['error'][$i] === UPLOAD_ERR_OK) {
                ...
                throw new Exception('Error uploading file: ' . $originalFileName . '. Error code: ' . $_FILES['files']['error'][$i]);
            }
        }
