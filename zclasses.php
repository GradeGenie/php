...
<body>
    ...
    <div id="mainContent">
        <h2>My Classes</h2>
        <p class="headingSubtitle">Pick up where you left off</p>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    var classes = response.classes.reverse(); // Reverse the order of the classes
                    classes.forEach(function(classItem) {
                        populateNewClass(classItem.cid, classItem.name, classItem.level, classItem.subject);
                    });
                } else {
    ...
    function populateNewClass(classId, className, level, subject) {
        var newClassCard = `
            <a href="view_assignments.php?id=${classId}">
                <div class="classCard">
                    <span class="classCard_name">${className}</span>
                    <div class="classCard_info">
                        <span class="classCard_level">${level}</span> • 
                        <span class="classCard_subject">${subject}</span>
                    </div>
                </div>
            </a>`;
        $('#classCardParent').prepend(newClassCard); // Use prepend instead of append
    }
    </script>
</body>
</html>
