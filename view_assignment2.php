...
<body>
    <div id="mainContent">
        ...
        <div class="actionButtons">
            ...
            <div>
                <button class="formCTA" onclick="showReportModal('download')">Generate Reports</button>
                <button class="formCTA" onclick="showReportModal('email')">Share via Email</button>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="assignment-list">
                    <div class="assignment-item">
                        <label>
                            <input type="checkbox" checked class="assignment-checkbox" value="<?php echo $_GET['id']; ?>">
                            Current Assignment
                        </label>
                    </div>
                    <!-- Other assignments will be loaded here -->
                </div>
                <div id="emailSection" style="display: none;">
                    <h3>Email Details</h3>
                    <input type="email" id="recipientEmail" class="email-input" placeholder="Enter recipient email">
                </div>
                <div class="modal-buttons">
                    <button class="formCTA cancel-button" onclick="closeReportModal()">Cancel</button>
                    <button class="formCTA" id="generateButton">Generate</button>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(assignments) {
                    ...
                    assignments.forEach(function(assignment) {
                        if (assignment.id != assignmentId) {  // Skip current assignment as it's already added
        ...
        // Show modal for generating reports
        window.showReportModal = function(mode) {
            isEmailMode = mode === 'email';
            $('#emailSection').toggle(isEmailMode);
            $('#generateButton').text(isEmailMode ? 'Send' : 'Generate');
            $('#reportModal').show();
        ...
        // Handle generate/send button click
        $('#generateButton').click(function() {
            ...
            if (isEmailMode) {
                ...
                window.location.href = `api/generate_reports.php?assignments=${selectedAssignments.join(',')}`;
                closeReportModal();
            }
        });

        // Add click handlers for the report buttons
        $('.actionButtons').html(`
            <div>
                <button class="formCTA" onclick="location.href='bulk_upload.php?assignmentId=${assignmentId}&classId=${classId}'">Upload Files</button>
                <button class="formCTA" onclick="exportCSV()">Export All</button>
            </div>
            <div>
                <button class="formCTA" onclick="showReportModal('download')">Generate Reports</button>
        ...
        function generateStudentReport() {
            ...
            fetch(`api/generate_report.php?studentId=${studentId}&assignmentId=${assignmentId}`)
        ...
        function emailReport() {
            ...
            fetch(`api/email_report.php?studentId=${studentId}&assignmentId=${assignmentId}`, {
                method: 'POST'
            })
